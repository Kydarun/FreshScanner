...
export default function HistoryDetail({ record, onToggleStorage, showStorageDetails }: HistoryDetailProps) {
  const { t } = useTranslation();
  const statusConfig = STATUS_CONFIG[record.freshness_status as FreshnessStatus] || STATUS_CONFIG['NOT_FOOD'];
  const StatusIcon = statusConfig.Icon;

  // Expiration and storage tracking variables
  let currentStorage: StorageEnvironment = 'FRIDGE';
  let daysLeft = -1;
  ...
  if (showStorageDetails && record.in_virtual_fridge) {
    currentStorage = record.current_storage || 'FRIDGE';
    daysLeft = record.estimated_shelf_life_days?.[currentStorage as keyof typeof record.estimated_shelf_life_days] || -1;
      ...
      {showStorageDetails && record.in_virtual_fridge && (
        <div className="glass-card p-5 flex flex-col gap-4 border border-white/10 shrink-0 bg-slate-950/40">
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold text-slate-400 uppercase tracking-wider">{t('storageMethod', 'Storage Method')}</span>
              ...
              <p className="text-xs text-slate-400 mt-0.5 min-w-0 leading-tight">
                {daysLeft === -1 ? (
                  ...
                  <span className="text-slate-300">
                    {t('expiresOn')} <strong className="text-white">{DateFormatter.formatDate(expiryDate!.getTime())}</strong> ({diffDays} {t('daysLeft', 'days left')})
                  </span>
                )}
              </p>
            </div>
